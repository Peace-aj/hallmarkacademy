...
interface NavigationMenuProps { links: MenuItem[]; logoSrc?: string; title?: string; }

const NavigationMenu: FC<NavigationMenuProps> = ({ links, logoSrc = '/assets/logo.png', title = 'Hallmark Academy Lafia' }) => {
          ...
          {links.map((item, idx) => (
            <li key={idx} className="relative">
              {item.submenu && item.sublinks ? (
                <>
                  <span
                    onMouseEnter={(e) => ops.current[idx]?.toggle(e)}
                    className={`cursor-pointer hover:text-gray-200 font-medium ${scrolled ? 'text-white' : 'text-gray-900'}`}
                  >
                    {item.name}
                  </span>
                  <OverlayPanel
                    ...
                    showCloseIcon={false}
                    dismissable={false}
                    className="!p-0 bg-white/30 backdrop-blur-md border-0 shadow-lg"
                ...
                <Link href={item.link || '#'} className={`hover:text-gray-200 font-medium ${scrolled ? 'text-white' : 'text-gray-900'}`}>
                  {item.name}
                </Link>
              )}
            </li>
            ...
            {links.map((item, idx) => (
              <li key={idx} className="border-b border-gray-700">
                {item.submenu && item.sublinks ? (
                      ...
                      {item.sublinks.map((group, gIdx) => (
                        <div key={gIdx}>
                          ...
                          <ul className="pl-4">
                            {group.sublink.map((sub, sIdx) => (
                              <li key={sIdx} className="py-1">
                                <Link href={sub.link} className="text-gray-200 hover:text-white text-sm block">
                                  {sub.name}
                                </Link>
                              </li>
                            ))}
                  ...
                  <Link href={item.link || '#'} className="block p-4 text-white hover:bg-gray-800">
                    {item.name}
                  </Link>
                )}
              </li>
